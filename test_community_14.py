...
"""
BEATRICE ROBSON
testing k-means on Nigeria hh data using item_cd 14: imported rice
"""
import numpy as np
import pandas as pd
...
source = pd.read_excel("community_14.xlsx", 
                       sheet_name="export")
...
while (stop == False):
    ...
    #last = points[:] #store for stopping condition
    ...
    for n in range(k):
        ...
        if counts[n] > 1:
            ...
            #points[n] = (np.random.rand(1) * sum(points) ) / k 
    ...
    stop = True
    for n in range(0,k):
        #print(counts[n], ": ", (last[n] - points[n]) / points[n] )
...
wb_indices = wb["indices_com"]
wb_means = wb["points_com"]
